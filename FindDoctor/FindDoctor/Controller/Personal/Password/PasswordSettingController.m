...
#import "PasswordSettingController.h"
...
@interface PasswordSettingController () <UITextFieldDelegate, UITextViewDelegate>

@end

@implementation PasswordSettingController
{
...
- (void)initContentView
{
    ...
    NSArray *titleArray = @[@"新密码",@"再输入新密码"];
    NSArray *viewArray = @[_newPassField,_newPassField1];
...
- (void)confirmAction
{
    ...
    [[CUUserManager sharedInstance] updateUser:[CUUserManager sharedInstance].user password:newPass verifyCode:self.verifyCode resultBlock:^(SNHTTPRequestOperation * request,SNServerAPIResultData * result) {
        [weakSelf hideProgressView];
        
        if (!result.hasError)
        {
            [TipHandler showHUDText:@"设置成功" inView:weakSelf.contentView];
            
            [weakSelf performSelector:@selector(backAction) withObject:nil afterDelay:0.7];
        }
        else {
            [TipHandler showHUDText:@"设置失败" inView:weakSelf.contentView];
        }
    } pageName:@"resetUserPassword"];
}
