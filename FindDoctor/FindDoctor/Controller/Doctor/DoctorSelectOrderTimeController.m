...
//        } pageName:@"DoctorSelectOrderTimeController"];
        ...
        [blockSelf postRequestGetUserMemberListWithData:selectOrderTime];
    };
}
...
//获取我的成员
- (void)postRequestGetUserMemberListWithData:(SelectOrderTime *)selectOrderTime{
    UserFilter * filter = [[UserFilter alloc] init];
    filter.listType = @"all";

    [[CUUserManager sharedInstance] getUserMemberListWithFilter:filter resultBlock:^(SNHTTPRequestOperation *request, SNServerAPIResultData *result) {
        ...
        if (!result.hasError) {
            ...
            if (![errorCode integerValue]) {
                ...
                VC.memberArray = listModel.items;
                
                CUOrder *order = [[CUOrder alloc]init];
                order.diagnosisID = selectOrderTime.orderID;
                order.service.doctor = self.doctor;
                VC.order = order;
