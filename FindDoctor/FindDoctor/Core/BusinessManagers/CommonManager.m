...
- (void)getActivityBannerWithFilter:(BannerFilter *)filter resultBlock:(SNServerAPIResultBlock)resultBlock pageName:(NSString *)pageName{
    ...
    [[AppCore sharedInstance].apiManager GET:[NSString stringWithFormat:@"/baseFrame/base/%@.jmm",[param stringForKeySafely:@"require"]]  parameters:nil callbackRunInGlobalQueue:NO parser:nil parseMethod:nil resultBlock:^(SNHTTPRequestOperation *request, SNServerAPIResultData *result){
        ...
        if (!result.hasError) {
            ...
            if (![errorCode integerValue]) {
                ...
                [tempList1 enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    banner.type = [[obj objectForKeySafely:@"type"] integerValue];
                    banner.redirectId = [[obj objectForKeySafely:@"redirectId"] integerValue];
