...
@interface SNTabViewController ()<SNTabBarDelegate>
{
...
@property (nonatomic, assign) SNViewController  *selectedViewController;
...
- (void)viewDidLoad
{
    ...
    self.customTabBar = [[[SNTabBar alloc] initWithFrame:tabBarFrame] autorelease];
    ...
    self.customTabBar.backgroundImage = [UIImage imageNamed:@"tabbar_background"];
...
{
    for (SNViewController *viewController in self.childViewControllers)
    {
...
- (SNViewController *)selectedViewController
{
        ...
        if ([self.viewControllers count])
        {
            if ([viewController isKindOfClass:[UINavigationController class]])
            {
                ...
                if (vc == [viewController visibleViewController])
                {
                    ...
                    {
                        [(id<SNTabBarTapDelegate>)vc tabViewControllerSingleTap:self];
                    }
                }
            }
            else
    ...
    else
    {
        ...
        if ([viewController isKindOfClass:[UINavigationController class]])
        {
            ...
            if (vc == [viewController visibleViewController])
            {
                ...
                {
                    [(id<SNTabBarTapDelegate>)vc tabViewControllerChangeSelectTap:self];
                }
            }
        }
        else
