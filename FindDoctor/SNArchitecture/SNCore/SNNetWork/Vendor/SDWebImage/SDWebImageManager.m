...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegate>)delegate
{
    ...
    {
        [cacheDelegates removeObjectAtIndexSafely:idx];
        [cacheURLs removeObjectAtIndexSafely:idx];
    }
    ...
    while ((idx = [downloadDelegates indexOfObjectIdenticalTo:delegate]) != NSNotFound)
    {
        ...
        [downloadInfo removeObjectAtIndexSafely:idx];
        [downloadDelegates removeObjectAtIndexSafely:idx];
        [downloaders removeObjectAtIndexSafely:idx];
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    // WARNING: add by nova
    if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:imageSize:)])
    {
        objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:imageSize:), self, image, url, 0);
    }
...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    /*
    if ([[CoreSinaNews sharedCore].cacheManager isImageExistOnDiskForNameKey:key]) {
        UIImage *image = [[CoreSinaNews sharedCore].cacheManager imageForKey:key];
        if (image) {
            [[SDImageCache sharedImageCache] storeImage:image forKey:key toDisk:NO];
            [self imageCache:imageCache didFindImage:image forKey:key userInfo:info];
        }
    }
    else*/
    {
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (downloader.imageData)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImageData:forURL:userInfo:)])
                {
                    objc_msgSend(delegate, @selector(webImageManager:didFinishWithImageData:forURL:userInfo:), self, downloader.imageData, downloader.url, userInfo);
                }
                
                // 保存
                if (downloader.imageData.length > 0)
                {
                    // Store the image in the cache
                    [[SDImageCache sharedImageCache] storeImageData:downloader.imageData
                                                             forKey:[self cacheKeyForURL:downloader.url]
                                                             forUrl:[downloader.url absoluteString]
                                                        forUserInfo:userInfo
            ...
            [downloaders removeObjectAtIndexSafely:uidx];
            [downloadInfo removeObjectAtIndexSafely:uidx];
            [downloadDelegates removeObjectAtIndexSafely:uidx];
        }
    }
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:imageSize:)])
                {
                    objc_msgSend(delegate, @selector(webImageManager:didFinishWithImage:forURL:imageSize:), self, image, downloader.url ,downloader.expectedSize);
                }
                if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage:forURL:userInfo:)])
                {
                    NSDictionary *userInfo = [[downloadInfo objectAtIndex:uidx] objectForKey:@"userInfo"];
