...
@synthesize url, delegate, connection, imageData, userInfo, lowPriority, progressive, expectedSize;
...
- (void)connection:(NSURLConnection *)aConnection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSRange range = [contentType rangeOfString:@"image/"];
    BOOL valid = (range.location != NSNotFound && range.length > 0);
    
    if ((![response respondsToSelector:@selector(statusCode)] || [((NSHTTPURLResponse *)response) statusCode] < 400) && valid)
    {
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:)])
    {
        ...
        if (width + height > 0 && totalSize < expectedSize)
        {
            ...
            if (partialImageRef)
            {
                ...
                {
                    [self imageDecoder:nil didFinishDecodingImage:image userInfo:nil];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)aConnection
{
    ...
    BOOL skipImage = NO;
    // WARNING:added by lina ，for need imageData not image
    if ([self.userInfo isKindOfClass:[NSDictionary class]])
    {
        NSDictionary * downloaderUserInfo = [self.userInfo objectForKey:@"userInfo"];
       if ([downloaderUserInfo isKindOfClass:[NSDictionary class]]
           && [downloaderUserInfo objectForKey:key_Downloader_UserInfo_NotifyImgData]
           && [delegate respondsToSelector:@selector(imageDownloaderDidFinish:)])
        {
            skipImage = YES;
            [delegate performSelector:@selector(imageDownloaderDidFinish:) withObject:self];
        }
    }
    if (!skipImage && [delegate respondsToSelector:@selector(imageDownloader:didFinishWithImage:)])
    {
        // modify by 马华  支持gif
