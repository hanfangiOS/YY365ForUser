...
NSString * Notification_ImageStoreDidFinished = @"Notification_ImageStoreDidFinished";
NSString * Key_ImageKey = @"Key_ImageKey";
NSString * Key_ImageUrl = @"Key_ImageUrl";
...
//        [memCache setTotalCostLimit:10 * 1024 * 1024]; // 10MB
//        [memCache setCountLimit:1];


        // Init the disk cache
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        NSString * thePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:@"TODO"];
        diskCachePath = SDWIReturnRetained([thePath stringByAppendingPathComponent:@"TODO"]);
...
- (void)storeKeyWithRawDataToDisk:(NSArray *)keyAndData
{
    // Can't use defaultManager another thread
    NSFileManager *fileManager = [[NSFileManager alloc] init];
    
    NSString *key = [keyAndData objectAtIndex:0];
    NSData *data = [keyAndData objectAtIndex:1];
    NSString * url = [keyAndData objectAtIndex:2];
    
    if (data)
    {
        [fileManager createFileAtPath:[self cachePathForKey:key] contents:data attributes:nil];
        NSMutableDictionary * dic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:key,Key_ImageKey,url,Key_ImageUrl, nil];
        [dic setValue:[keyAndData objectAtIndex:3] forKey:@"userInfo"];
        [[NSNotificationCenter defaultCenter] postNotificationNameOnMainThread:Notification_ImageStoreDidFinished object:nil userInfo:dic];
        [dic release],dic = nil;
                ...
                toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        if (data)
        {
            keyWithData = [NSArray arrayWithObjects:key, data, url,userInfo, nil];
            
            NSInvocationOperation *operation = SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
                                                                                                           selector:@selector(storeKeyWithRawDataToDisk:)
                                                                                                             object:keyWithData]);
...
{
    [self storeImage:image imageData:data forKey:key toDisk:YES toMemory:YES];
}
...
//    [cacheOutQueue cancelAllOperations];
//    NSInvocationOperation *operation = SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self
//                                                                                                   selector:@selector(queryDiskCacheOperation:)
//                                                                                                     object:arguments]);
//    [cacheOutQueue addOperation:operation];
    [NSThread detachNewThreadSelector:@selector(queryDiskCacheOperation:) toTarget:self withObject:arguments];
}
...
-(int)getSize
{
    return [NSFileManager directorySizeAtPath:diskCachePath];
